...
export default function VideoUploader({ onUploadComplete, onUploadError }: VideoUploaderProps) {
  ...
  return (
    <div className="w-full">
      {/* Idle State - 파일 선택 */}
      {status === 'idle' && (
        <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-gray-300 rounded-xl cursor-pointer hover:border-pink-400 hover:bg-pink-50 transition-colors">
          <div className="flex flex-col items-center justify-center pt-5 pb-6">
            <svg className="w-8 h-8 mb-2 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
            </svg>
            <p className="text-sm text-gray-500">
              <span className="font-medium text-pink-500">클릭하여 영상 업로드</span>
            </p>
      ...
      {/* Uploading State */}
      {status === 'uploading' && (
        <div className="w-full p-4 border-2 border-pink-200 rounded-xl bg-pink-50">
          ...
          <div className="flex items-center justify-between mt-2">
            ...
            <span className="text-xs font-medium text-pink-600">{progress}%</span>
          </div>
        </div>
      )}
      ...
      {/* Processing State */}
      {status === 'processing' && (
        <div className="flex items-center justify-center w-full h-32 border-2 border-purple-200 rounded-xl bg-purple-50">
          <div className="flex items-center gap-3">
