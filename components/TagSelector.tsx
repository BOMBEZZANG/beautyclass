      ...
      <div className="flex flex-wrap gap-2 min-h-[40px] p-3 bg-gray-50 rounded-xl border border-gray-200">
        {selectedTags.length === 0 ? (
          ...
          getSelectedTagInfo().map(tag => (
            <span
              key={tag.id}
              className="inline-flex items-center gap-1 px-3 py-1.5 bg-gradient-to-r from-pink-500 to-purple-500 text-white rounded-full text-sm font-medium"
            >
              #{tag.name}
              <button
                type="button"
              ...
              return (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-full text-sm font-medium transition-all ${
                    isSelected
                      ? 'bg-pink-100 text-pink-700 border-2 border-pink-400 shadow-sm'
                      : isDisabled
                      ? 'bg-gray-100 text-gray-300 cursor-not-allowed'
                      : 'bg-gray-100 text-gray-600 border-2 border-transparent hover:border-pink-200 hover:bg-pink-50'
                  }`}
                >
                  #{tag.name}
                </button>
