    ...
    <div className="p-6 lg:p-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        ...
        <Link
          ...
          className="inline-flex items-center justify-center gap-2 rounded-full bg-gradient-to-r from-pink-500 to-purple-500 px-6 py-3 text-white font-medium hover:shadow-lg transition-shadow"
        >
            ...
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${
                statusFilter === item.value
                  ? item.value === 'published' ? 'bg-green-500 text-white' : item.value === 'draft' ? 'bg-yellow-500 text-white' : 'bg-pink-500 text-white'
      ...
      {courses.length === 0 ? (
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-12 text-center">
          ...
          <Link
            href="/instructor/courses/new"
            className="inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-pink-500 to-purple-500 px-8 py-4 text-white font-medium hover:shadow-lg transition-shadow"
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
            </svg>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {filteredCourses.map((course) => (
            <div
              key={course.id}
              className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-lg hover:border-pink-200 transition-all group"
            >
              {/* 썸네일 */}
              ...
              <div className="p-5">
                ...
                <div className="flex gap-2">
                  <Link
                    ...
                    className="flex-1 text-center py-2 px-4 bg-pink-50 text-pink-600 rounded-xl font-medium text-sm hover:bg-pink-100 transition-colors"
                  >
                    편집하기
                  </Link>
                  <Link
